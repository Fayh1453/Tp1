...
public class Facture {
	...
	private double[] prixTable = new double[NOMBRE_TABLE+1];
	...
	public void lireFacture() {
		...
		if (texte[i].equalsIgnoreCase("Clients:")) {
			...
			while (!texte[i].equalsIgnoreCase("Plats:")) {
				...
				if (verif.verifierClient(texteClient[0])
						&& verif.verifierTable(texteClient[1], NOMBRE_TABLE)) {
					clientTemp = new Clients(texteClient[0],
							Integer.parseInt(texteClient[1]));
					tabClients[j++] = clientTemp;
				} else {
					erreur += System.getProperty("line.separator")
							+ "Le nom du client ou le numéro de table "
							+ texte[i] + " n'est pas valide.";
					--compteurC;
			...
			while (!texte[i].equalsIgnoreCase("Commandes:")) {
				...
				if (verif.verifierPlat(textePlat[0])
						&& verif.verifierPrix(textePlat[1])) {
					Plat platTemp = new Plat(textePlat[0],
							Double.parseDouble(textePlat[1]));
					tabPlats[k++] = platTemp;
				} else if (!verif.verifierPlat(textePlat[0])) {
					erreur += System.getProperty("line.separator")
							+ "Le nom du plat " + textePlat[0]
							+ " n'est pas valide.";
					--compteurP;
				} else {
					erreur += System.getProperty("line.separator")
							+ "Le prix du plat " + textePlat[1]
							+ " n'est pas valide.";
					--compteurP;
				}
				++i;
			...
			while (!texte[i].equalsIgnoreCase("Fin")) {
				...
				if (verif.verifierClient(texteCommandes[0])
						&& verif.verifierPlat(texteCommandes[1])
						&& verif.verifierQuantite(texteCommandes[2])) {
					Commandes commandesTemp = new Commandes(texteCommandes[0],
							texteCommandes[1],
							Double.parseDouble(texteCommandes[2]));
					tabCommandes[l++] = commandesTemp;
				} else if (!verif.verifierClient(texteCommandes[0])) {
					erreur += System.getProperty("line.separator")
							+ "Le nom du client " + texteCommandes[0]
							+ " n'est pas valide.";
					--compteurCommandes;
				} else if (!verif.verifierPlat(texteCommandes[1])) {
					erreur += System.getProperty("line.separator")
							+ "Le nom du plat " + texteCommandes[1]
							+ " n'est pas valide.";
					--compteurCommandes;
				} else {
					erreur += System.getProperty("line.separator")
							+ "La quantité " + texteCommandes[2]
							+ " n'est pas valide.";
					--compteurCommandes;
				}
				++i;
	...
	public void affichage() throws IOException {
		...
		File fichier = new File("Facture-du-" + dateFormat.format(date)
				+ ".txt");
		String enregistrement = "Bienvenue chez Barette!"
				+ System.getProperty("line.separator") + "Factures:"
		...
		for (int i = 0; i < prixTable.length; ++i) {
			...
			if (prixTable[i] > 0 && prixTable[i] < 100) {
				...
				if (compteurClientTable >= 3) {
					prixTable[i] *= 1.15;
					prixTable[i] += calculTPS(prixTable[i])
							+ calculTVQ(prixTable[i]);
					System.out.println("Numéro de table " + i + ": "
							+ df.format(prixTable[i]) + "$");
				} else {
					prixTable[i] += calculTPS(prixTable[i])
							+ calculTVQ(prixTable[i]);
					System.out.println("Numéro de table " + i + ": "
	...
	public static void enregistrer(String enregistrement, File fichier)
			throws IOException {
